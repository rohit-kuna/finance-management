...
export async function getManageImportExportData(): Promise<ManageImportExportDataDto> {
  ...
  const [organization, categoriesResult, counterparties, members] = await Promise.all([
    getOrganizationById(orgId),
    getCategoriesByOrg(orgId),
    getCounterpartiesByOrg(orgId),
    getOrganizationMembers(orgId),
  ]);
  const transactionModesByMember = await Promise.all(
    members.map(async (member) => getTransactionModesByUser(member.id))
  );
  ...
  return {
    ...
    counterparties,
    transactionModes: transactionModesByMember.flat(),
    members,
    currentUser: {
      id: currentUser.id,
      role: currentUser.role,
...
export async function parseImportWorkbookAction(
  ...
  try {
    ...
    const preview = parseWorkbookBuffer(buffer, file.name);
...
export async function importExpensesFromWorkbookAction(
  ...
  for (const row of payload.rows) {
    ...
    } else if (userSelection.error) {
      issues.push(userSelection.error);
    }
    ...
    if (counterpartySelectionValue) {
      ...
      } else if (counterpartySelection.error) {
        issues.push(counterpartySelection.error);
      }
    }
    ...
    if (modeSelectionValue) {
      ...
      } else if (modeSelection.error) {
        issues.push(modeSelection.error);
      }
    }
    ...
    try {
      ...
      issues.push(error instanceof Error ? error.message : `Invalid amount: ${amountValue}`);
    }

    try {
      parseNecessityScore(necessityScoreValue);
    } catch (error) {
      issues.push(error instanceof Error ? error.message : `Invalid necessity_score: ${necessityScoreValue}`);
    }
    ...
    try {
      ...
      issues.push(error instanceof Error ? error.message : `Invalid scope: ${scopeValue}`);
    }
    ...
    try {
      ...
      issues.push(error instanceof Error ? error.message : `Invalid transactionTimestamp: ${timestampValue}`);
    }
